...
const LAYOUT_DEFAULTS = (boldpo.layoutDefaults) || { container_width: '1200px' };
...
export default function Settings() {
    ...
    const initialContainerWidth = parseContainerWidth(
        (boldpo.layout && boldpo.layout.container_width) || LAYOUT_DEFAULTS.container_width
    );
    const [containerWidth, setContainerWidth] = useState(initialContainerWidth);

    const applyContainerWidthToRoot = (px) => {
        if (px === '' || px == null) return;
        document.documentElement.style.setProperty('--boldpo-layout-row-max-width', `${px}px`);
    };
    ...
    const handleSave = () => {
        ...
        Promise.allSettled([
            ...
            .then(([colorsRes, layoutRes]) => {
                ...
                if (colorsOk) boldpo.colors = colorsRes.value.colors || colors;
                if (layoutOk) boldpo.layout = layoutRes.value.layout || { container_width: containerWidthValue };
    ...
    return (
        <div className="boldpo-options-content">
            ...
            <p style={{ marginTop: 0, color: '#555' }}>
                Sets the boxed content max-width used by the BoldPost Row block. Stored as
                the CSS variable <code>--boldpo-layout-row-max-width</code> on <code>:root</code>.
            </p>
