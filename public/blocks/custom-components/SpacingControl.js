...
const sideFullLabel = (s) => ({
    top:    __('Top', 'boldpost'),
    right:  __('Right', 'boldpost'),
    bottom: __('Bottom', 'boldpost'),
    left:   __('Left', 'boldpost'),
}[s]);
...
const SpacingControl = ({
    ...
    return (
                ...
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', width: '100%' }}>
                    ...
                    <div style={{ display: 'flex', alignItems: 'center', gap: 4 }}>
                        <SelectControl
                            ...
                            size="compact"
                            hideLabelFromVision
                            label={__('Unit', 'boldpost')}
                            __nextHasNoMarginBottom
                        />
                        <Tooltip text={linked ? __('Unlink sides', 'boldpost') : __('Link sides', 'boldpost')}>
                            <Button
                                size="small"
                                variant="tertiary"
                                icon={linked ? 'admin-links' : 'editor-unlink'}
                                onClick={() => setLinked((v) => !v)}
                                aria-label={linked ? __('Unlink sides', 'boldpost') : __('Link sides', 'boldpost')}
                            />
                        </Tooltip>
                    </div>
                </div>
        ...
        >
            <div
                className="boldpo-spacing-control"
                style={{
                    display: 'grid',
                        ...
                        <Tooltip key={side} text={sideFullLabel(side)}>
                            ...
                            >
                                <NumberControl
                                    ...
                                    className="boldpo-spacing-control__input"
                                />
                            </div>
                        </Tooltip>
                    );
