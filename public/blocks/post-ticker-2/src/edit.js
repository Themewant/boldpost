	...
	useEffect(() => {
		const init = () => {
			if (typeof window.initBoldpoSliderPostTicker2 === 'function') {
				window.initBoldpoSliderPostTicker2(blockRef.current);
			}
		};
		...
		<div {...useBlockProps({ ref: blockRef })}>
			...
			<InspectorControls>
				...
				<PanelBody title={__('Content', 'boldpost')} initialOpen={false}>
					<TextControl
						label={__('Ticker Label', 'boldpost')}
				...
				<PanelBody title={__('Label', 'boldpost')} initialOpen={false}>
					...
					<ResponsiveWrapper label={__('Padding', 'boldpost')}>
						{(device) => (
							<BoxControl
								values={attributes[getAttrKey('labelPadding', device)]}
								onChange={(value) => setAttributes({ [getAttrKey('labelPadding', device)]: value })}
							/>
						)}
					</ResponsiveWrapper>
					<Divider />
					<ResponsiveWrapper label={__('Text Align', 'boldpost')}>
						{(device) => (
							<TextAlignControl
								attributes={attributes}
								setAttributes={setAttributes}
								attributeKey={getAttrKey('labelTextAlign', device)}
							/>
						)}
					</ResponsiveWrapper>
					<Divider />
					<ResponsiveWrapper label={__('Typography', 'boldpost')}>
						{(device) => (
							<TypographyControls
								...
								attributes={attributes}
								setAttributes={setAttributes}
								attributeKey={getAttrKey('labelTypography', device)}
							/>
						)}
					</ResponsiveWrapper>
				</PanelBody>
			...
			<ServerSideRender block="boldpost/post-ticker-2" attributes={attributes} httpMethod="POST" />
		</div>
	);
}