		...
		<div {...useBlockProps()}>
			...
			<InspectorControls>
				...
				<PanelBody title={__('Thumbnail', 'boldpost')} initialOpen={false}>
					...
					<ResponsiveWrapper
						label={attributes.gridStyle === '1' ? __('Thumbnail Right Height', 'boldpost') : __('Thumbnail Height', 'boldpost')}
					>
						{(device) => (
							<RangeControlWithUnit
								attributes={attributes}
			...
			<ServerSideRender block="boldpost/post-grid-4" attributes={attributes} httpMethod="POST" />
		</div >
	);
}
