(function (window) {
    ...
    window.initBoldpoSliderTwo = function (scope) {
        ...
        wraps.forEach(function (wrap) {
            ...
            const sliderElement = wrap.querySelector('.boldpo-post-slider-2-' + unique);
                ...
                new Swiper(sliderElement, {
                    ...
                    on: {
                        init: function () {
                            ...
                            // Add fade-in to active slide title on init
                            const activeSlide = this.slides[this.activeIndex];
                            if (activeSlide) {
                                const title = activeSlide.querySelector('.boldpo-blog-content');
                                if (title) title.classList.add('fade-in');
                            }
                        },
                        slideChangeTransitionStart: function () {
                            // Remove fade-in from all slide titles
                            this.slides.forEach(slide => {
                                const title = slide.querySelector('.boldpo-blog-content');
                                if (title) title.classList.remove('fade-in');
                            });
                            // Add fade-in to the new active slide title
                            const activeSlide = this.slides[this.activeIndex];
                            if (activeSlide) {
                                const title = activeSlide.querySelector('.boldpo-blog-content');
                                if (title) title.classList.add('fade-in');
