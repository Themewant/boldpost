...
const ALLOWED_BLOCKS = ['boldpost/column'];
...
const presetIcon = (preset) => {
    ...
    return (
        <span className="boldpo-preset-icon" aria-hidden="true">
            {preset.columns.map((w, i) => (
                <span
                    key={i}
                    className="boldpo-preset-icon__col"
...
export default function Edit({ attributes, setAttributes, clientId }) {
    ...
    const innerBlocksProps = useInnerBlocksProps(
        { className: 'boldpo-layout-row__inner' },
        {
    ...
    const applyPreset = useCallback(
        (newPresetId) => {
            ...
            const merged = widths.map((w, idx) => {
                ...
                return createBlock('boldpost/column', { width: widthStr });
            });
    ...
    const addColumn = useCallback(() => {
        ...
        const next = [
            ...(innerBlocks || []),
            createBlock('boldpost/column', { width: evenWidth }),
        ];
    ...
    return (
        <>
            ...
            <BlockControls>
                <ToolbarGroup>
                    <ToolbarButton icon="plus-alt2" label={__('Add Column', 'boldpost')} onClick={addColumn} />
                    <ToolbarButton
                        icon="minus"
                        label={__('Remove Column', 'boldpost')}
                ...
                <PanelBody title={__('Layout', 'boldpost')} initialOpen={true}>
                    <div className="boldpo-preset-grid">
                        {LAYOUT_PRESETS.map((p) => (
                            <Button
                                key={p.id}
                                className={`boldpo-preset-btn ${preset === p.id ? 'is-active' : ''}`}
                                onClick={() => applyPreset(p.id)}
                                label={p.label}
                            >
                                {presetIcon(p)}
                                <span className="boldpo-preset-btn__label">{p.label}</span>
                            </Button>
                        ))}
                    </div>
                    <Divider />
                    ...
                    <SelectControl
                        label={__('Content Width', 'boldpost')}
                        value={contentWidth}
                        options={[
                            { label: __('Boxed', 'boldpost'), value: 'boxed' },
                    ...
                    {contentWidth === 'boxed' && (
                        <ResponsiveWrapper label={__('Max Width', 'boldpost')}>
                            {(device) => (
                                <UnitControl
                ...
                <PanelBody title={__('Advanced Layout', 'boldpost')} initialOpen={false}>
                    <SelectControl
                        ...
                        options={[
                            ...
                            { label: __('Top', 'boldpost'), value: 'top' },
                            { label: __('Middle', 'boldpost'), value: 'middle' },
                            { label: __('Bottom', 'boldpost'), value: 'bottom' },
                        ]}
                    ...
                    <Divider />
                    <ToggleControl
                        label={__('Equal Height Columns', 'boldpost')}
                        checked={equalHeight}
                    ...
                    <Divider />
                    <ToggleControl
                        label={__('Stretch Columns', 'boldpost')}
            ...
            <InspectorControls group="styles">
                <PanelBody title={__('Background', 'boldpost')} initialOpen={false}>
                    ...
                    <Divider />
                    <TextControl
                        label={__('Background Image URL', 'boldpost')}
            ...
            <Tag {...blockProps}>
                {hasChildren ? (
                    ...
                    <div className="boldpo-layout-row__layout-picker">
                        <div className="boldpo-layout-row__layout-picker__title">
                            {__('Chose a Layout', 'boldpost')}
                        </div>
                        <div className="boldpo-layout-row__layout-picker__grid">
                            {LAYOUT_PRESETS.map((p) => (
                                <button
                                    type="button"
                                    key={p.id}
                                    className="boldpo-layout-row__layout-picker__item"
                                    onClick={() => applyPreset(p.id)}
                                    aria-label={p.label}
                                    title={p.label}
                                >
