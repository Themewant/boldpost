...
$unique_id     = ! empty( $attributes['blockId'] )
    ...
    : 'boldpo-layout-row-' . wp_rand( 100, 99999 );
...
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'flexDirection',  'flex-direction' );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'justifyContent', 'justify-content' );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'alignItems',     'align-items' );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'alignContent',   'align-content' );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'flexWrap',       'flex-wrap' );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'gap',            'gap' );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'rowGap',         'row-gap' );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'columnGap',      'column-gap' );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'minHeight',      'min-height' );

// Padding / margin (object, D/T/M).
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'padding', '', [
    'top'    => 'padding-top',
    'right'  => 'padding-right',
    'bottom' => 'padding-bottom',
    'left'   => 'padding-left',
], true );
BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'margin', '', [
    'top'    => 'margin-top',
    'right'  => 'margin-right',
...
if ( ! empty( $radius['top'] ) )    $row_responsive['desktop']['border-top-left-radius']     = BOLDPO_Helper::ensure_unit( $radius['top'] );
if ( ! empty( $radius['right'] ) )  $row_responsive['desktop']['border-top-right-radius']    = BOLDPO_Helper::ensure_unit( $radius['right'] );
if ( ! empty( $radius['bottom'] ) ) $row_responsive['desktop']['border-bottom-right-radius'] = BOLDPO_Helper::ensure_unit( $radius['bottom'] );
if ( ! empty( $radius['left'] ) )   $row_responsive['desktop']['border-bottom-left-radius']  = BOLDPO_Helper::ensure_unit( $radius['left'] );

// Box shadow.
if ( ! empty( $attributes['boxShadow'] ) && ! empty( $attributes['boxShadow']['c'] ) && $attributes['boxShadow']['c'] !== 'rgba(0,0,0,0)' ) {
    $row_responsive['desktop']['box-shadow'] = BOLDPO_Helper::box_shadow_to_css( $attributes['boxShadow'] );
}
...
foreach ( [ '' => 'desktop', 'Tablet' => 'tablet', 'Mobile' => 'mobile' ] as $sfx => $dev ) {
    ...
    foreach ( [ 'columnGap', 'gap' ] as $base ) {
        ...
        if ( $val !== '' ) {
            $row_responsive[ $dev ]['--bp-gap'] = BOLDPO_Helper::ensure_unit( $val );
            break;
        }
    }
}
...
// in style.scss already reads it on the inner via var(--boldpo-layout-row-max-width).
...
if ( $content_width === 'boxed' ) {
    BOLDPO_Helper::add_responsive_vars( $attributes, $row_responsive, 'maxWidth', '--boldpo-layout-row-max-width' );
}

// Compile CSS.
$style_handle = 'boldpo-layout-row-style';
$css  = BOLDPO_Helper::generate_responsive_css( $selector, $row_responsive );
...
// Output.
printf(
    '<%1$s %2$s><div class="boldpo-layout-row__inner">%3$s</div></%1$s>',
    tag_escape( $tag ),
