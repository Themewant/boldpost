...
export default function Edit({ attributes, setAttributes }) {
	...
	const paginationOptions = applyFilters(
		'boldpost.post-grid-3.pagination_options',
		[
	...
	const dev = useSelect((select) => {
		...
		let device = 'Desktop';
		if (editor && typeof editor.__experimentalGetPreviewDeviceType === 'function') {
			device = editor.__experimentalGetPreviewDeviceType();
		} else if (editor && typeof editor.getDeviceType === 'function') {
			device = editor.getDeviceType();
		}
	...
	return (
		<div {...useBlockProps()}>
			<BlockControls>
				...
				<ToolbarDropdownMenu
					icon="heading"
					label={__('Left Title Tag', 'boldpost')}
					text={(attributes.titleLeftTag || 'h3').toUpperCase()}
					controls={['h2', 'h3', 'h4', 'h5', 'h6'].map((t) => ({
						title: t.toUpperCase(),
						isActive: attributes.titleLeftTag === t,
						onClick: () => setAttributes({ titleLeftTag: t }),
					}))}
				/>
				<ToolbarDropdownMenu
					icon="heading"
					label={__('Middle Title Tag', 'boldpost')}
					text={(attributes.titleMiddleTag || 'h3').toUpperCase()}
					controls={['h2', 'h3', 'h4', 'h5', 'h6'].map((t) => ({
						title: t.toUpperCase(),
						isActive: attributes.titleMiddleTag === t,
						onClick: () => setAttributes({ titleMiddleTag: t }),
					}))}
				/>
				<ToolbarDropdownMenu
					icon="heading"
					label={__('Right Title Tag', 'boldpost')}
					text={(attributes.titleRightTag || 'h3').toUpperCase()}
					controls={['h2', 'h3', 'h4', 'h5', 'h6'].map((t) => ({
						title: t.toUpperCase(),
						isActive: attributes.titleRightTag === t,
						onClick: () => setAttributes({ titleRightTag: t }),
					}))}
				/>
			</BlockControls>
			<InspectorControls>
				...
				<PanelBody title={__('Thumbnail', 'boldpost')} initialOpen={false}>
					<SelectControl
						...
						__nextHasNoMarginBottom={true}
					/>
					<ResponsiveWrapper label={__('Thumbnail Left Height', 'boldpost')}>
						{(device) => (
							<RangeControlWithUnit
								attributes={attributes}
								setAttributes={setAttributes}
								attributeKey={getAttrKey('thumbnailLeftHeight', device)}
					...
					<ResponsiveWrapper label={__('Thumbnail Middle Height', 'boldpost')}>
						{(device) => (
							<RangeControlWithUnit
								attributes={attributes}
								setAttributes={setAttributes}
								attributeKey={getAttrKey('thumbnailMiddleHeight', device)}
								units={['px', '%', 'em', 'rem', 'vw', 'vh']}
								min={0}
								max={1000}
								step={1}
					...
					<ResponsiveWrapper label={__('Thumbnail Right Height', 'boldpost')}>
						{(device) => (
							<RangeControlWithUnit
								attributes={attributes}
								setAttributes={setAttributes}
								attributeKey={getAttrKey('thumbnailRightHeight', device)}
								units={['px', '%', 'em', 'rem', 'vw', 'vh']}
								min={0}
								max={1000}
								step={1}
					...
					<ResponsiveWrapper label={__('Typography Left', 'boldpost')}>
						{(device) => (
							<TypographyControls
								...
								attributes={attributes}
								setAttributes={setAttributes}
								attributeKey={getAttrKey('itemTitleLeftTypography', device)}
							/>
						)}
					</ResponsiveWrapper>
			...
			<ServerSideRender block="boldpost/post-grid-3" attributes={attributes} httpMethod="POST" />
		</div>
	);
}
