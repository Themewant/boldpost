...
export default function Edit({ attributes, setAttributes }) {
	...
	useEffect(() => {
		...
		const tryInit = () => {
			...
			if (
				blockRef.current &&
				typeof window.initBoldpoSlider4 === 'function' &&
				typeof window.Swiper !== 'undefined' &&
				blockRef.current.querySelector('.boldpo-post-slider-4-block-wrap')
			) {
				window.initBoldpoSlider4(blockRef.current);
			} else {
		...
		<div {...useBlockProps({ ref: blockRef })}>
			<BlockControls>
				...
				<ToolbarDropdownMenu
					...
					text={(attributes.titleTag || 'h6').toUpperCase()}
			...
			<InspectorControls>
						...
						{(tab) => {
							...
							return (
								<div style={{ marginTop: '15px' }}>
									...
									/>
									{/* Optional override for the main (left) slide title. Left empty it
									    keeps using the Color above, so saved posts are unaffected. */}
									<ColorPopover
										label={__('Left Title Color', 'boldpost')}
										color={isHover ?
											attributes.titleLeftColorHover
											: attributes.titleLeftColor}
										defaultColor=""
										onChange={(value) => {
											const hex = (value && typeof value === 'object') ? value.hex : value;
											setAttributes({ [isHover ? 'titleLeftColorHover' : 'titleLeftColor']: hex });
										}}
									/>
								</div>
							);
				...
				{['1', '2'].includes(attributes.sliderStyle) && (
			...
			<ServerSideRender block="boldpost/post-slider-4" attributes={attributes} httpMethod="POST" />
		</div>
	);
}