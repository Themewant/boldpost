...
export default function Edit({ attributes, setAttributes }) {
	...
	const FREE_PAGINATION_VALUES = ['numeric', 'next_prev'];
	...
	let paginationOptions = [
		...
		{ label: __('Next / Prev', 'boldpost'), value: 'next_prev' },
	];

	paginationOptions = applyFilters('boldpost.post-list-4.pagination_options', paginationOptions);
		...
		<div {...useBlockProps()}>
			...
			<InspectorControls group='styles'>
				...
				<PanelBody title={__('Pagination', 'boldpost')} initialOpen={false}>
					...
					{
						attributes.paginationType == 'next_prev' && (
							<>
								<Divider />
								<BoxControl
									label={__('Prev Button Margin', 'boldpost')}
									values={attributes.prevButtonMargin}
									onChange={(value) => setAttributes({ prevButtonMargin: value })}
								/>
								<BoxControl
									label={__('Next Button Margin', 'boldpost')}
									values={attributes.nextButtonMargin}
									onChange={(value) => setAttributes({ nextButtonMargin: value })}
								/>
							</>
						)
					}
