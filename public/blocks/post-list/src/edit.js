...
export default function Edit({ attributes, setAttributes }) {
	...
	const dev = useSelect((select) => {
		const editor = select('core/edit-post') || select('core/edit-site') || select('core/editor');
		let deviceType = 'Desktop';
		if (editor) {
			if (typeof editor.getDeviceType === 'function') {
				deviceType = editor.getDeviceType();
			} else if (typeof editor.__experimentalGetPreviewDeviceType === 'function') {
		...
		<div {...useBlockProps()}>
			...
			<InspectorControls>
					...
					<ImageRadioControl
						...
						options={[
							...
							...(isLicenseActive ? [
								...
								{ label: __('Style 7', 'boldpost'), value: '7', src: layout8 },
							] : []),
						]}
					/>
				</PanelBody>
				...
				<PanelBody title={__('Pagination', 'boldpost')} initialOpen={false}>
					...
					<ResponsiveWrapper label={__('Button Height', 'boldpost')}>
						{(device) => (
							<RangeControlWithUnit
								attributes={attributes}
								setAttributes={setAttributes}
								attributeKey={getAttrKey('paginationBtnHeight', device)}
								units={['px', 'em', 'rem', 'vw', 'vh']}
								min={0}
								max={500}
								step={1}
					...
					<BoxShadowControl
						label={__('Box Shadow', 'boldpost')}
						value={attributes.paginationBoxShadow}
						onChange={(value) => setAttributes({ paginationBoxShadow: value })}
					/>
					<Divider />
					<ResponsiveWrapper label={__('Padding', 'boldpost')}>
						{(device) => (
			...
			<ServerSideRender block="boldpost/post-list" attributes={attributes} httpMethod="POST" />
		</div>
	);
}